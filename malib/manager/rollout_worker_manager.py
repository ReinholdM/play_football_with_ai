...
class RolloutWorkerManager:
    def __init__(
        ...
        benchmark_env_desc: Dict[str, Any],
        ...
        worker_num = (
            len(possible_agents)
            if worker_config["worker_num"] == -1
            else worker_config["worker_num"]
        )
        print("got worker num:", worker_num)
        ...
        for i in range(worker_num):
            ...
            self._workers[worker_idx] = worker_cls.options(max_concurrency=100).remote(
                ...
                benchmark_env_desc=benchmark_env_desc,
    ...
    def test(self, task_desc: TaskDescription) -> None:
        """Parse rollout task and dispatch it to available worker.
        ...
        # split into several sub tasks rollout
        worker_idx, worker = self.get_idle_worker()
        ray.get(worker.test.remote(task_desc))

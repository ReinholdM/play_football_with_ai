...
class TrainingManager:
    def __init__(
        ...
        self._initial_policy_config = initial_policy_config
        ...
        for env_aid in env_desc["possible_agents"]:
            ...
            for training_aid in training_aids:
                if training_aid not in self._agents:
                    print(f"Create Agent {training_aid}, env_aid={env_aid}")
    ...
    def init(self) -> None:
        ...
        if self._initial_policy_config is not None:
            print("############## initial_policy key in!")
            # print(self._initial_policy_config)
            for p_items in self._initial_policy_config:
                policy_dir = Path(p_items["policy_dir"])
                policy_cfg_pickle = pickle.load(open(policy_dir/"desc.pkl", "rb"))
                policy_cls = get_algorithm_space(policy_cfg_pickle["registered_name"]).policy

                policy = policy_cls.load(policy_dir)
                print(f"Load Policy from {policy_dir}")
                for aid, agent_interface in self._agents.items():
                    ray.get(agent_interface.add_init_policy.remote(p_items["policy_id"], policy))
        tasks = []
        for aid, agent_interface in self._agents.items():
            tasks.append(

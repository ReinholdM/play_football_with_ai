...
DATASET_TABLE_NAME_GEN = lambda env_id, main_id, pid: f"{main_id}_{pid}"
    ...
    def insert(self, **kwargs):
        ...
        for column in self.columns:
            if isinstance(kwargs[column], np.ndarray):
                # print(f"insert column: {column}, shape: {kwargs[column].shape}")
...
class Table:
    ...
    def sample(self, idxes=None, size=None, threshold: int = 0) -> Any:
        with self._threading_lock:
            if self._episode.size < threshold:
                print("NoEnoughData")
                raise NoEnoughDataError
            data = self._episode.sample(idxes, size)
            assert idxes is not None, "idxes should not be None"
            self.sample_idx = (self.sample_idx + len(idxes)) % self._episode.size
        return data
...
class OfflineDataset:
    ...
    def save(self, agent_episodes: Dict[AgentID, Episode], wait_for_ready: bool = True):
        ...
        insert_results = []
        if isinstance(agent_episodes, list):
            for agent_episode in agent_episodes:
                for aid, episode in agent_episode.items():
                    ...
                    # print(f"DataSet Insert: {aid}, {episode.size}, "
                    #     f"table_size:{self._tables[table_name].size}"
                    #     f", table_cap:{self._tables[table_name].capacity}")
        else:
            for aid, episode in agent_episodes.items():
                ...
                self._tables[table_name].insert(**episode.data)
                self.logger.debug(
                    f"Threads created for insertion on table={table_name}"
                )
    ...
    @Log.method_timer(enable=settings.PROFILING)
    def sample(
        self, agent_buffer_desc_dict: Dict[AgentID, BufferDescription], alignment=False
    ) -> Any:
        ...
        try:
            ...
            with Log.timer(log=settings.PROFILING, logger=self.logger):
                min_size, batch_size, candidates = [], [], []
                for agent, buffer_desc in agent_buffer_desc_dict.items():
                    ...
                    table = self._tables[table_name]
                    min_size.append(table.size)
                    batch_size.append(buffer_desc.batch_size)
                    candidates.append((agent, table))
                if alignment:
                    batch_size = min(batch_size)
                    min_size = min(min_size)
                    idxes = np.random.choice(min_size, batch_size)

                    for agent, table in candidates:
                        res[agent] = table.sample(
                            idxes=idxes, threshold=self._learning_start
                        )
                else:
                    for agent, table in candidates:
                        buffer_desc = agent_buffer_desc_dict[agent]
                        res[agent] = table.sample(
                            idxes=np.arange(
                                table.sample_idx,
                                min(table.size, table.sample_idx + buffer_desc.batch_size)
                            )
                        )

        except (KeyError, OversampleError, NoEnoughDataError) as e:
            info = f"data table `{table_name}` not ready"
            res = None
            print(info)
        return res, info

...
AgentFeedback = namedtuple("AgentFeedback", "id, trainable_pairs, state_id, statistics, trainable")
...
class AgentInterface(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self._num_init_policy = 0

    def add_init_policy(self, policy_id, policy):
        assert policy_id not in self._policies
        self._policies[policy_id] = policy
        self._num_init_policy += 1
        assert len(self._group) == 1
        env_aid = self._group[0]
        with self._param_desc_lock:
            self._agent_to_pids[env_aid].append(policy_id)
            param_desc = self.parameter_desc_gen(
                env_aid, policy_id, trainable=False, data=policy.state_dict()
            )
            pending_task = self._parameter_server.push.remote(parameter_desc=param_desc)
            param_desc.data = None
            self._parameter_desc[policy_id] = param_desc
            if env_aid not in self._meta_parameter_desc:
                self._meta_parameter_desc[env_aid] = MetaParameterDescription(
                    meta_pid=env_aid, parameter_desc_dict={}
                )
            self._meta_parameter_desc[env_aid].parameter_desc_dict[policy_id]\
                 = self.parameter_desc_gen(env_aid, policy_id, trainable=False)
        status = ray.get(pending_task)

        task_request = TaskRequest(
            task_type=TaskType.SIMULATION,
            content=AgentFeedback(
                id=self._id,
                trainable_pairs={env_aid: (policy_id, policy.description)},
                statistics={},
                state_id=None,
                trainable=False
            )
    ...
    def as_remote(
        ...
        # print(f"agent_cls.as_remote: GPU:{num_gpus}")
        return ray.remote(
            num_cpus=num_cpus,
            num_gpus=num_gpus,
            memory=memory,
    ...
    def train(self, task_desc: TaskDescription, training_config: Dict[str, Any] = None):
        ...
        training_policy_id = next(iter(policy_id_mapping.values()))
        ...
        while not stopper(statistics, global_step=epoch) and not stopper.all():
            ...
            self.logger.send_scalar(
                tag=f"Train/{self._id}/{training_policy_id}/time_get_data",
                content=t1 - t0,
                global_step=epoch,
            )
            ...
            with Log.stat_feedback(
                ...
                t1 = time.time()
                self.logger.send_scalar(
                    tag=f"Train/{self._id}/{len(self._policies)}/time_backward",
                    content=t1 - t0,
                    global_step=epoch,
                )
                ...
                if epoch % training_config["saving_interval"] == 0:
                    ...
                    self.policies[training_policy_id].dump(dump_dir)
                    print(f"Epoch {epoch}: a copy of policy {training_policy_id}" 
                          f"has been dumped to {dump_dir}")
    ...
    def add_policy(self, task_desc: TaskDescription):
        ...
        if len(self.policies) <= self._num_init_policy:
            trainable = False
        else:
            trainable = True
        ...
        policy_dict: Dict[AgentID, Tuple[PolicyID, Policy]] = {
            ...
            for env_aid in sorted(self._group)
        }
    ...
    def default_policy_id_gen(self, algorithm_conf: Dict[str, Any]) -> str:
        ...
        return f"{algorithm_conf['name']}_{len(self._policies)-self._num_init_policy}"
    ...
    def optimize(
        ...
        :param Dict[AgentID,Any] batch: A dict of agent batch

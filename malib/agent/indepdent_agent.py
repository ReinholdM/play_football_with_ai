"""
Independent agent interface, for independent algorithms training.
"""
    ...
    def optimize(
        ...
        for env_aid, pid in policy_ids.items():
            ...
            if env_aid not in batch_copy:
                print(f"[WARNING] no available batch in this batch named {env_aid}")
                continue
            trainer.reset(self._policies[pid], training_config)
            # FIXME(ming): require to handling multiagent episode return (which has nested episodes)

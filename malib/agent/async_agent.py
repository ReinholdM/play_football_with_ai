...
class AsyncAgent(IndependentAgent):
    ...
    def optimize(
        ...
        for env_aid, pid in policy_ids.items():
            ...
            trainer.reset(self.policies[pid], training_config=training_config)
            res[env_aid] = trainer.optimize(batch[env_aid])
            gradients = trainer.loss.stacked_gradients
            assert (
                len(gradients) > 0
            ), "No available gradients transferred from loss function. Make sure the loss.step executed."
            res[env_aid] = metrics.to_metric_entry(res[env_aid])
            self._cumulative_grads[pid].extend(gradients)

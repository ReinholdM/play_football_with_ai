...
class TeamCTDEAgent(AgentInterface):
    ...
    def __init__(
        ...
        self._generation_buffer = set()
        self._cur_generation = 0
    ...
    def add_policy_for_agent(
        ...
        :param bool trainable: Specify whether the added policy is trainable or not
        ...
        pid += f"_{self._cur_generation}"

        self._generation_buffer.add(env_agent_id)
        if len(self._generation_buffer) == len(self._group):
        ...
        if pid in self.policies:
            ...
            all_action_spaces = {aid: self._action_spaces[aid] for aid in self._group}
            all_observation_spaces = {
                aid: self._observation_spaces[aid] for aid in self._group
            }
            custom_config.update(
                {
                    "global_state_space": gym.spaces.Dict(
                        {
                            "observations": gym.spaces.Dict(**all_observation_spaces),
                            "actions": gym.spaces.Dict(**all_action_spaces),
                        }
                    )
                }
            )
